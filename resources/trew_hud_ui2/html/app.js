...
window.onload = function () {
		...
		var eventCallback = {
			ui: function(data) {
				...
				else if (config.showServerLogo == false) { document.querySelector('.info.player').style.top = '-32px'; }
			...
			setStatus: function(data) {
				if (data.isdead == true) {
					if (document.querySelector('#health').classList.contains('dead') == false) {
						...
						for (i = 0; i < data.status.length; i++) { saferInnerHTML(document.querySelector('#'+data.status[i].name+' span'), '0%'); }
					}
				}else {
					for (i = 0; i < data.status.length; i++) {
						...
						if (document.querySelector('#'+data.status[i].name+' span')) { saferInnerHTML(document.querySelector('#'+data.status[i].name+' span'), statusValue+'%'); }
					}
				}
			},
			...
			updateVehicle: function(data) {
				...
				var vehicleRPM = document.querySelector('#vehicle-geare');
				var vehicleKM = document.querySelector('#km');
				...
				if (data.status == true) {
					if (vehicleInfo.classList.contains('inactive')) {
						...
						if (vehiclesCars.indexOf(data.type) > -1) {
							...
							document.querySelector('#vehicle-km').style.display = 'block';
							...
							document.querySelector('#vehicle-km').style.display = 'none';
					...
					saferInnerHTML(vehicleKM, data.km+"km");

					if(data.RPM > 100){
						data.RPM = 100;
					}
					vehicleRPM.style.height = data.RPM+'%';
					...
					if (vehicleLights.classList.contains(data.lights) == false) {
						...
						if (data.lights == 'high') { vehicleLights.querySelector('i img').src = 'img/duga.png'; }
						else { vehicleLights.querySelector('i img').src = 'img/kratka.png'; }
					}
					...
					if (vehicleSignals.classList.contains(data.signals) == false) {
						...
						if (data.signals == 'left') { 
							vehicleSignals.querySelector('i img').src = 'img/lijevi.png'; 
							vehicleSignals.classList.add(data.signals, 'dying');
							eventCallback.sound('sounds/car-indicators.ogg', { loop: 'loop' });
						}
						else if (data.signals == 'right') { 
							vehicleSignals.querySelector('i img').src = 'img/desni.png'; 
							vehicleSignals.classList.add(data.signals, 'dying');
							eventCallback.sound('sounds/car-indicators.ogg', { loop: 'loop' });
						}
						else if (data.signals == 'both'){ 
							vehicleSignals.querySelector('i img').src = 'img/zmigavci.png'; 
							vehicleSignals.classList.add(data.signals, 'dying');
							eventCallback.sound('sounds/car-indicators.ogg', { loop: 'loop' });
						}
		...
		window.addEventListener('message', function(event) {
			var item = event.data;
			if (item.prikazi) {
				var x = document.getElementById("tamonesta");
				var y = document.getElementById("money");
				var z = document.getElementById("job");
				x.style.top = "0px";
				y.style.left = "4px";
