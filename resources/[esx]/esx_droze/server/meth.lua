...
AddEventHandler('esx_illegal:processMeth', function()
	if not playersProcessingMeth[source] then
		...
		playersProcessingMeth[_source] = ESX.SetTimeout(Config.Delays.MethProcessing, function()
			...
			local xhydrochloric_acid,xsulfuric_acid,xsodium_hydroxide = xPlayer.getInventoryItem('hydrochloric_acid'),xPlayer.getInventoryItem('sulfuric_acid'),xPlayer.getInventoryItem('sodium_hydroxide')
