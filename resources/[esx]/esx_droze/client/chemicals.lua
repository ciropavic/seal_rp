	...
	for k, v in pairs(ESX.GetPlayerData().inventory) do
		...
		if price and v.count >= 0 then
			table.insert(elements, {
				label = ('%s  <span style="color:green;">%s</span>'):format("Napravi " .. v.label , ""),
				name = v.name,
			})
		end
	end
...
RegisterNetEvent('kemikalija:EoTiGaFreeze')
AddEventHandler('kemikalija:EoTiGaFreeze', function(br)
	FreezeEntityPosition(PlayerPedId(), br)
end)
