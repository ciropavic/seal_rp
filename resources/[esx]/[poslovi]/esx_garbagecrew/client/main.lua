...
function DodajBlip()
	...
	AddTextComponentString("Smetlar")
...
AddEventHandler('esx:setPosao', function(job)
	...
	if IsJobgarbage() then
		...
		if not IsJobgarbage() and blip ~= nil then
			RemoveBlip(blip)
			blip = nil
		end
	end
...
function IsJobgarbage()
	if PlayerData ~= nil then
		...
		if PlayerData.posao.name ~= nil and PlayerData.posao.name == 'garbage' then
			isJobgarbage = true
		end
		return isJobgarbage
	end
...
RegisterNUICallback(
    ...
    function(data, cb)
		...
		if br == 1 then
			...
			ESX.ShowNotification("Zaposlio si se kao smetlar!")
		end
    end
)
...
AddEventHandler('esx_garbagejob:hasEnteredMarker', function(zone)
	...
	if zone == "posao" then
		TriggerEvent("upit:OtvoriPitanje", "esx_garbagecrew", "Upit za posao", "Dali se zelite zaposliti kao smetlar?")
	end
...
function retourcamionperdu_oui()
	...
	if PVozilo ~= nil and DoesEntityExist(PVozilo) then
		ESX.Game.DeleteVehicle(PVozilo)
		PVozilo = nil
	end
						...
						if dist <= 3.5 then
							if currentbag > 0 then
								...
								if iscurrentboss then
									...
									trashcollection = false
									truckdeposit = false
									ESX.ShowNotification("Kupljenje zavrseno, vratite se u vozilo!")
	...
	while true do
		...
		if IsJobgarbage() then
			if truckdeposit then
				...
				dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, trunk.x , trunk.y, trunk.z)
				if dist <= 2.0 then
				ESX.Game.Utils.DrawText3D(vector3(trunk.x , trunk.y ,trunk.z + 0.50), "[~g~E~s~] Bacite vrecu u kamion.", 1.0)
				end
			end
			...
			if trashcollection then
				...
				if dist <= 5.0 then
					if currentbag <= 0 then
						if iscurrentboss then
						ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Ocistite kontenjer", 1.0)		
						else
						ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "Kupljenje zavrseno.. Pricekajte u kamionu..", 1.0)		
						end
					else
						ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "[~g~E~s~] Pokupite vrecu iz smeca ["..currentbag.."/"..bagsoftrash.."]", 1.0)
					end
				end
			end
