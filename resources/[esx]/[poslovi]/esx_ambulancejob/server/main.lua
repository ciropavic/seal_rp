...
RegisterServerEvent('ambu:ozivi')
AddEventHandler('ambu:ozivi', function(target)
	...
	if xPlayer.job.name == 'ambulance' then
		...
		societyAccount.addMoney(3500)
		TriggerClientEvent('ambu:ozivi', target)
	else
		print(('esx_ambulancejob: %s attempted to revive!'):format(xPlayer.identifier))
		DropPlayer(source, "Citer")
	end
end)
...
ESX.RegisterServerCallback('ambulanta:kradenakonsmrti', function(source, cb)
	local xPlayer = ESX.GetPlayerFromId(source)
...
AddEventHandler('esx_ambulancejob:giveItem', function(itemName, torba)
	...
	if xPlayer.job.name ~= 'ambulance' then
		...
		return
	elseif (itemName ~= 'medikit' and itemName ~= 'bandage'  and itemName ~= "repairkit") then
		print(('esx_ambulancejob: %s attempted to spawn in an item!'):format(xPlayer.identifier))
		return
	end
...
TriggerEvent('es:addGroupCommand', 'revive', 'admin', function(source, args, user)
	if args[1] ~= nil then
		if GetPlayerName(tonumber(args[1])) ~= nil then
			...
			TriggerClientEvent('ambu:ozivi', tonumber(args[1]))
		end
	else
		TriggerClientEvent('ambu:ozivi', source)
	end
