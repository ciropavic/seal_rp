...
RegisterServerEvent("inventory:tradetuljana")
AddEventHandler(
	"inventory:tradetuljana",
	function(from, target, type, itemName, itemCount)
		...
		elseif type == "item_money" then
			if itemCount > 0 and sourceXPlayer.getMoney() >= itemCount then
				...
				TriggerEvent("DiscordBot:Inventory", GetPlayerName(_source).." je dao igracu "..GetPlayerName(target).." $"..itemCount)
