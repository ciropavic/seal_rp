	...
	if tonumber(args[1]) and tonumber(args[2]) and tonumber(args[3]) then
		...
		if xPlayer then
			if ESX.DoesJobExist(tonumber(args[2]), args[3]) then
				...
				TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Lista mafiji/orgi:' } })
				for i=1, #ESX.Jobs, 1 do
					TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'ID: '..ESX.Jobs[i].pID.." | Ime: "..ESX.Jobs[i].label } })
				end
			end
		else
			TriggerClientEvent('chat:addMessage', source, { args = { '^1SYSTEM', 'Igrac nije online.' } })
