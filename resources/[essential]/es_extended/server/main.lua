		...
		table.insert(tasks, function(cb)
			...
			table.insert(tasks2, function(cb2)
				...
				MySQL.Async.fetchAll('SELECT ID, job, job_grade, posao, loadout, position, phone_number, firstname, lastname, firma, status FROM `users` WHERE `identifier` = @identifier', {
					...
					if ESX.DoesPosaoExist(posao) then
						local jobObject = ESX.Poslovi[tonumber(posao)]

						userData.posao = {}

						userData.posao.id    = jobObject.pID
						userData.posao.name  = jobObject.name
						userData.posao.label = jobObject.label
					else
						print(('es_extended: %s had an unknown job [job: %s], setting as unemployed!'):format(player.getIdentifier(), posao))

						local job = 1
						local jobObject = ESX.Poslovi[job]
					...
					if result[1].phone_number ~= nil then
						userData.number = result[1].phone_number
					end
		...
		Async.parallel(tasks, function(results)
			...
			--xPlayer.getMissingAccounts(function(missingAccounts)
				...
				TriggerClientEvent('esx:playerLoaded', _source, {
					...
					job          = xPlayer.getJob(),
					posao 		 = xPlayer.getPosao(),
					ID 			 = xPlayer.getID(),
					ime			 = xPlayer.getRPName(),
